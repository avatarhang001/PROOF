/**
 * Home — reference-matched PROOF learning dashboard.
 */
import { api } from '../api.js';
import { app, refreshMe } from '../state.js';
import { esc, $, ico, toast, timeAgo, fmtNim } from '../ui.js';
import { generateAndOpenPath } from './generate.js';
import { walletEntry } from './onboarding.js';

export async function screen(root) {
  root.innerHTML = `<div class="reference-home"><div class="skeleton" style="height:180px;border-radius:15px"></div></div>`;
  let d;
  try { d = await api.get('/api/home'); } catch { location.hash = '#/onboarding'; return; }
  const u = d.user || {};
  const first = esc((u.username || 'learner').split(/\s+/)[0]);
  const streak = Number(u.streak?.current || 0);
  const nim = fmtNim(u.balanceNim || 0, 1);
  const skills = Array.isArray(d.mySkills) ? d.mySkills : [];
  const daily = d.daily || { title: 'Complete a proof checkpoint', rewardNim: 1, xp: 100, done: false };
  const cont = d.continueLearning;
  const percent = Math.max(0, Math.min(100, Number(cont?.percent || 0)));
