...
const ROUTES = [
  ...
  ['', home.screen], ['prove', prove.challengeScreen], ['prove/challenge/:id', prove.challengeScreen], ['prove/attempt/:id', prove.attemptScreen],
...
function setBadge(link, count) {
  ...
  if (count > 0) {
    ...
    badge.textContent = count > 99 ? '99+' : count > 9 ? '9+' : String(count);
...
function updateNavBadges() {
  ...
  if (!nav) return;
  setBadge(nav.querySelector('a[href="#/reviews"]'), reviewsDueCount);
  setBadge(nav.querySelector('a[href="#/notifications"]'), notificationsUnreadCount);
}

function renderNav(activeKey) {
  if (!navRendered) {
    const nav = document.createElement('nav');
    nav.className = 'nav'; nav.id = 'bottomNav'; nav.setAttribute('aria-label', 'Primary navigation');
    nav.innerHTML = NAV.map((n) => `<a href="${n.href}" data-key="${n.match[0]}" data-mobile="${n.mobile}" aria-label="${n.label}"><span class="nav-ico" data-ico="${n.ico}"></span><span class="nav-label">${n.label}</span></a>`).join('');
    appEl().after(nav); navRendered = true; updateReviewsCount(); setInterval(updateReviewsCount, 300000);
  }
  const nav = document.getElementById('bottomNav');
  nav.querySelectorAll('a').forEach((a) => {
    const key = a.dataset.key; const item = NAV.find((n) => n.match[0] === key); if (!item) return;
    a.classList.toggle('active', item.match.includes(activeKey));
    const span = a.querySelector('.nav-ico'); if (span && !span.dataset.done) { span.innerHTML = ico[item.ico]; span.dataset.done = '1'; }
  });
  updateNavBadges();
}
...
(async function boot() { WalletService.restore(); const me = await refreshMe(); if (!me) location.hash = location.hash.startsWith('#/u/') ? location.hash : '#/onboarding'; app.booted = true; router(); })();
export { api, app };
