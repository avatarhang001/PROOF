...
export async function hub(root) {
  ...
  if (walletStatusEl) {
    walletStatusEl.innerHTML = walletStatusBadge(app.me?.wallet?.mode, app.me?.walletModeIsDemo);
  }
  ...
  root.querySelectorAll('[data-skill]').forEach((n) => n.addEventListener('click', () => {
    ...
    // Directly create a path for this skill without showing the goal modal
    const defaultGoal = `I want to learn ${skillName.toLowerCase()}`;
    generateAndOpenPath({ goal: defaultGoal, domain: slug, anchor: document.getElementById('app') }).catch((e) => toast(esc(e.message), 'bad'));
  }));
...
async function tutorChat(skillSlug, topicSlug, lesson) {
  ...
  const history = [];
  const s = sheet(`
    <div class="proof-tutor-modal" role="dialog" aria-modal="true" aria-label="AI Tutor">
      <div class="proof-tutor-header">
        <div class="proof-tutor-avatar">
          ${ico.sparkle.replace('<svg', '<svg width="20" height="20"')}
        </div>
        <div>
          <b>PROOF Tutor</b>
          <div class="tiny">${esc(lesson.title)} · guides, never just answers</div>
        </div>
        <button class="proof-tutor-close" id="closeChat" aria-label="Close tutor">${ico.x}</button>
      </div>
      <div class="proof-tutor-chat" id="chatLog">
        <div class="proof-tutor-msg proof-tutor-msg--ai">
          <div class="proof-tutor-msg-avatar">${ico.sparkle.replace('<svg', '<svg width="14" height="14"')}</div>
          <div class="proof-tutor-msg-bubble">Hey — I'm your tutor for <b>${esc(lesson.title)}</b>. Ask me anything, or pick a quick action below:</div>
        </div>
      </div>
      <div class="proof-tutor-quick" id="quick">
        <button class="proof-tutor-quick-btn" data-q="Explain it simpler">${ico.sparkle} Explain simpler</button>
        <button class="proof-tutor-quick-btn" data-q="Show me an example">${ico.code} Example</button>
        <button class="proof-tutor-quick-btn" data-q="Give me an exercise">${ico.flask} Exercise</button>
        <button class="proof-tutor-quick-btn" data-q="I'm stuck, give me a hint">${ico.bulb} Hint</button>
      </div>
      <div class="proof-tutor-input-wrap">
        <input class="proof-tutor-input" id="chatInput" placeholder="Ask about this lesson…" />
        <button class="proof-tutor-send" id="chatSend" aria-label="Send message">${ico.send}</button>
      </div>
    </div>
  `, { });
  ...
  const sparkleIcon = ico.sparkle.replace('<svg', '<svg width="14" height="14"');
  
  const send = async (q) => {
    if (!q.trim()) return;
    log.append(el(`<div class="proof-tutor-msg proof-tutor-msg--user"><div class="proof-tutor-msg-bubble">${esc(q)}</div></div>`));
    input.value = '';
    const typing = el(`<div class="proof-tutor-msg proof-tutor-msg--ai"><div class="proof-tutor-msg-avatar">${sparkleIcon}</div><div class="proof-tutor-msg-bubble proof-tutor-typing"><i></i><i></i><i></i></div></div>`);
    log.append(typing);
    ...
    try {
      ...
      typing.remove();
      log.append(el(`<div class="proof-tutor-msg proof-tutor-msg--ai"><div class="proof-tutor-msg-avatar">${sparkleIcon}</div><div class="proof-tutor-msg-bubble">${esc(r.reply)}</div></div>`));
      log.scrollTop = log.scrollHeight;
    } catch (e) {
      typing.remove();
      log.append(el(`<div class="proof-tutor-msg proof-tutor-msg--ai"><div class="proof-tutor-msg-avatar">${sparkleIcon}</div><div class="proof-tutor-msg-bubble" style="color:var(--bad)">⚠️ ${esc(e.message)}</div></div>`));
      log.scrollTop = log.scrollHeight;
    }
  };
  ...
  s.el.querySelectorAll('.proof-tutor-quick-btn').forEach((c) => c.addEventListener('click', () => send(c.dataset.q)));
...
export async function documentUploadScreen(root) {
  root.innerHTML = `<div class="pad" style="padding-top:max(14px, env(safe-area-inset-top))">
    <div class="row-between">
      <button class="btn btn-ghost btn-sm" id="back">${ico.back} Learning</button>
      <div id="walletStatusPlaceholder"></div>
    </div>
    
    <div class="card card-hero mt16" style="padding:32px 24px;text-align:center">
      <div style="font-size:48px;margin-bottom:16px">${ico.upload}</div>
      <h1 class="h1" style="color:#fff;margin:0;font-size:22px">Create Curriculum from Document</h1>
      <p class="sub mt10" style="color:rgba(255,255,255,.82);font-size:15px;line-height:1.5">Upload a PDF, Word doc, or text file — AI will analyze it and build a personalized learning path with lessons, quizzes, and proof challenges.</p>
    </div>

    <div class="section mt16">
      <div class="section-head"><span class="eyebrow">YOUR DOCUMENT CURRICULA</span></div>
      <div id="docList" class="stack"></div>
    </div>

    <div class="section">
      <div class="section-head"><span class="eyebrow">UPLOAD NEW DOCUMENT</span></div>
      <form id="uploadForm" class="card" style="padding:24px">
        <div class="field">
          <label class="label">Select Document</label>
          <div class="proof-file-dropzone" id="dropzone">
            <input type="file" id="fileInput" accept=".pdf,.docx,.txt" hidden />
            <div class="proof-dropzone-content">
              ${ico.file}<br>
              <span class="proof-dropzone-text">Drag & drop or click to select</span>
              <span class="proof-dropzone-hint">PDF, DOCX, or TXT · Max 10MB</span>
            </div>
            <div class="proof-file-selected" id="fileSelected" hidden></div>
          </div>
        </div>
        <div class="field">
          <label class="label" for="docGoal">Your Goal (optional)</label>
          <input type="text" id="docGoal" class="input" placeholder="e.g., Master this for a job interview, Build a project with this knowledge..." maxlength="240" />
        </div>
        <button type="submit" class="btn btn-primary btn-block mt16" id="uploadBtn" disabled>
          ${ico.sparkle} Analyze & Create Curriculum
        </button>
        <p class="tiny center mt8" style="color:var(--muted)">Your curriculum will be private and unique to you. The document is processed securely and not stored after analysis.</p>
      </form>
    </div>
  </div>`;
  ...
  const fileSelected = root.querySelector('#fileSelected');
  const uploadBtn = root.querySelector('#uploadBtn');
  let selectedFile = null;

  dropzone.addEventListener('click', () => fileInput.click());
  dropzone.addEventListener('dragover', (e) => { e.preventDefault(); dropzone.classList.add('proof-dropzone-active'); });
  dropzone.addEventListener('dragleave', () => dropzone.classList.remove('proof-dropzone-active'));
  dropzone.addEventListener('drop', (e) => {
    e.preventDefault();
    dropzone.classList.remove('proof-dropzone-active');
  ...
  function handleFile(file) {
    ...
    if (!validTypes.includes(file.type) && !validExts.includes(ext)) {
      toast('Unsupported file type. Please upload PDF, DOCX, or TXT.', 'bad');
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      toast('File too large. Maximum size is 10MB.', 'bad');
      return;
    }

    selectedFile = file;
    fileInput.value = '';
    dropzone.classList.add('proof-dropzone-has-file');
    fileSelected.hidden = false;
    fileSelected.innerHTML = `
      <div class="proof-selected-file">
        ${ext === '.pdf' ? ico.file : ext === '.docx' ? ico.file : ico.code}
        <div>
          <div class="proof-selected-name">${esc(file.name)}</div>
          <div class="proof-selected-size">$(file.size / 1024 / 1024).toFixed(2) MB</div>
        </div>
        <button type="button" class="proof-selected-remove" id="removeFile">${ico.x}</button>
      </div>`;
    uploadBtn.disabled = false;
    root.querySelector('#removeFile')?.addEventListener('click', (e) => {
      e.stopPropagation();
      clearFile();
    });
  }
  ...
  function clearFile() {
    selectedFile = null;
    dropzone.classList.remove('proof-dropzone-has-file');
  ...
  root.querySelector('#uploadForm').addEventListener('submit', async (e) => {
    ...
    uploadBtn.disabled = true;
    uploadBtn.innerHTML = `${ico.sparkle} Analyzing document…`;
    
    try {
      const formData = new FormData();
      formData.append('document', selectedFile);
      formData.append('goal', root.querySelector('#docGoal').value.trim());

      const res = await fetch('/api/curriculum/from-document', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
      ...
      toast('Curriculum created! 🎓', 'ok');
      clearFile();
      root.querySelector('#docGoal').value = '';
      location.hash = `#/learn/path/${path.id}`;
    } catch (err) {
      uploadBtn.disabled = false;
      uploadBtn.innerHTML = `${ico.sparkle} Analyze & Create Curriculum`;
      toast(esc(err.message), 'bad');
    }
  });

  // Load existing document curricula
  loadDocumentList();

  async function loadDocumentList() {
    const listEl = root.querySelector('#docList');
    try {
      const { paths } = await api.get('/api/curriculum/documents');
      if (!paths.length) {
        listEl.innerHTML = `<div class="card" style="text-align:center;padding:32px">
          <div style="font-size:32px;margin-bottom:12px">📄</div>
          <b>No document curricula yet</b>
          <p class="sub mt8">Upload your first document to create a personalized learning path.</p>
        </div>`;
        return;
      }
      listEl.innerHTML = paths.map(p => `
        <div class="card card-click proof-doc-item" data-path="${p.id}" style="padding:16px 18px">
          <div class="row-between">
            <div class="row" style="gap:12px;align-items:center;flex:1;min-width:0">
              <div class="avatar av-40" style="background:var(--nim-soft);color:var(--nim-deep)">${ico.file}</div>
              <div style="min-width:0">
                <b style="font-size:15px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(p.title)}</b>
                <div class="tiny" style="color:var(--muted)">${p.skillEmoji} ${esc(p.skillName)} · ${p.days?.length || 0} days · ${p.totalXp} XP</div>
              </div>
            </div>
            <span class="chip chip-nim">${ico.calendar} ${new Date(p.createdAt).toLocaleDateString()}</span>
          </div>
        </div>
      `).join('');
      
      root.querySelectorAll('.proof-doc-item').forEach(item => {
        item.addEventListener('click', () => { location.hash = `#/learn/path/${item.dataset.path}`; });
      });
    } catch (err) {
      console.error('Failed to load document curricula:', err);
      listEl.innerHTML = `<div class="card" style="text-align:center;padding:32px"><p class="sub">Failed to load your document curricula.</p></div>`;
    }
  }
}
