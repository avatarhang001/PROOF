...
export async function challengeScreen(root, { id }) {
  ...
  root.innerHTML = `<div class="pad" style="padding-top:max(14px, env(safe-area-inset-top))">
    ...
    <div class="card mt16">
      ...
      ${ch.requirements?.length ? `<div class="mt16"><span class="eyebrow" style="color:var(--ink-2)">REQUIREMENTS</span><ul class="reqs requirements-list">${ch.requirements.map((r, i) => `<li data-req-index="${i}" data-met="false" data-touched="false"><span class="tick">✓</span> ${esc(r)}</li>`).join('')}</ul></div>` : ''}
    ...
    <div class="mt20">
      ${isCode ? `<div class="field"><label class="label">Your ${ch.type === 'html' ? 'HTML' : 'code'}</label>
        <div class="input-wrapper">
          ...
          <span class="char-count" data-for="fCode" data-max="10000">0 / 10,000</span>
        </div></div>` : ''}
      ${fields.includes('explanation') ? `<div class="field"><label class="label">Edge cases — how did you handle them? <span class="tiny">(2–3 sentences)</span></label>
        <div class="input-wrapper">
          <textarea id="fExpl" class="input" style="min-height:120px" placeholder="Empty input, wrong types, boundaries…"></textarea>
          <span class="char-count" data-for="fExpl" data-max="500">0 / 500</span>
        </div></div>` : ''}
      ${fields.includes('text') ? `<div class="field"><label class="label">Your submission <span class="tiny">(<span id="wc">0</span> words)</span></label>
        <div class="input-wrapper">
          <textarea id="fText" class="input" style="min-height:220px;font-size:15px;line-height:1.6" placeholder="${esc(textPlaceholder(ch))}"></textarea>
          <span class="char-count" data-for="fText" data-max="5000">0 / 5,000</span>
        </div></div>` : ''}
  ...
  if (walletStatusEl2) {
    walletStatusEl2.innerHTML = walletStatusBadge(app.me?.wallet?.mode, app.me?.walletModeIsDemo);
  }
  ...
  $$('[data-for]', root).forEach((counter) => {
    ...
    const updateCounter = () => {
      ...
      counter.dataset.exceeded = len > max;
      if (len > max) {
  ...
  if (ch.requirements?.length) {
    const reqItems = root.querySelectorAll('.requirements-list li[data-req-index]');
    const textareas = [$('#fCode', root), $('#fText', root), $('#fExpl', root)].filter(Boolean);
    
    const checkRequirements = () => {
      const fullText = textareas.map(t => t.value).join('\n').toLowerCase();
      
      reqItems.forEach((item) => {
        const reqText = item.textContent.toLowerCase().replace('✓', '').trim();
        const keywords = reqText.split(/\s+/).filter(w => w.length > 3);
        
        let met = false;
        if (keywords.length) {
          met = keywords.every(kw => fullText.includes(kw.toLowerCase()));
        } else {
          // Fallback: check if requirement text appears in submission
          met = fullText.includes(reqText.substring(0, 20));
        }
        
        const wasMet = item.dataset.met === 'true';
    ...
    textareas.forEach(ta => {
      ta.addEventListener('input', debouncedCheck);
      ta.addEventListener('blur', () => {
        // Mark all as touched on blur
        reqItems.forEach(item => item.dataset.touched = 'true');
      });
    });
...
export async function attemptScreen(root, { id }) {
  ...
  root.innerHTML = `<div class="pad center bento-read" style="padding-top:max(28px, env(safe-area-inset-top))">
    ...
    ${extra?.newAchievements?.length ? `<div class="card mt12 center" style="background:linear-gradient(120deg,rgba(233,178,19,.16),rgba(233,178,19,.06))">${extra.newAchievements.map((a) => `<div><b>${a.emoji} ${esc(a.name)}</b><div class="tiny" style="color:var(--nim-deep)">${esc(a.desc)}</div></div>`).join('<div class="divider" style="margin:8px 0"></div>')}</div>` : ''}
  ...
  if (walletStatusEl3) {
    walletStatusEl3.innerHTML = walletStatusBadge(app.me?.wallet?.mode, app.me?.walletModeIsDemo);
  }
