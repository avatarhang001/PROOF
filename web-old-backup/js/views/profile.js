...
export async function screen(root) {
  ...
  root.innerHTML = `<div class="pad" style="padding-top:max(14px, env(safe-area-inset-top))">
    <div class="row-between">
      ...
      <div class="proof-profile-actions">
        <button class="proof-profile-menu-btn" id="profileMenuBtn" aria-label="More options" aria-expanded="false">
          ${ico.moreHorizontal}
        </button>
        <a href="#/reviews" class="proof-profile-icon-btn" aria-label="Review queue">${ico.clock.replace('<svg', '<svg width="20" height="20"')}</a>
        <a href="#/notifications" class="proof-profile-icon-btn" aria-label="Notifications">${ico.bell.replace('<svg', '<svg width="20" height="20"')}${app.unread ? `<span class="proof-notification-badge">${app.unread > 9 ? '9+' : app.unread}</span>` : ''}</a>
      </div>
    </div>

    <div class="proof-profile-menu" id="profileMenu" role="menu" hidden>
      <a href="#/leaderboard" class="proof-menu-item" role="menuitem">${ico.trophy}<span>Leaderboard</span></a>
      <a href="#/glossary" class="proof-menu-item" role="menuitem">${ico.book}<span>Glossary</span></a>
      <a href="#/socratic" class="proof-menu-item" role="menuitem">${ico.chat}<span>Socratic Tutor</span></a>
      <a href="#/settings" class="proof-menu-item" role="menuitem">${ico.settings}<span>Settings</span></a>
    </div>
  ...
  // Profile menu toggle
  const menuBtn = root.querySelector('#profileMenuBtn');
...
function streakCard(streak) {
  ...
  return `<div class="card mt12" style="padding:16px;background:linear-gradient(135deg, rgba(233,178,19,.1) 0%, #FFF 100%);border:1px solid rgba(233,178,19,.35)">
    <div class="row" style="gap:12px;align-items:center">
      ...
      <div class="stat" style="background:var(--surface);padding:10px 12px;min-width:60px">
